...
describe('FinicityConnect', () => {
  let mockWindow;
  beforeEach(() => {
    mockWindow = { close: jest.fn(), focus: jest.fn(), closed: false, postMessage: jest.fn() };
    jest.useFakeTimers();
  });
  ...
  describe('destroy', () => {
    test('should remove iframe and meta elements', () => {
      const iframeStub: any = { parentNode: { removeChild: jest.fn() }, setAttribute: jest.fn() };
      const metaStub: any = { parentNode: { removeChild: jest.fn() }, setAttribute: jest.fn() };
      spyOn(document.head, 'appendChild').and.callFake(() => { });
      spyOn(document.body, 'appendChild').and.callFake(() => { });
      spyOn(window.document, 'createElement').and.callFake((element) => element === 'iframe' ? iframeStub : metaStub);
      jest.spyOn(window, 'removeEventListener');
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { } });
      FinicityConnect.destroy();
    ...
    test('should remove postMessage event listener', () => {
      let onMessageFn;
      window.addEventListener = (cb) => { onMessageFn = cb };
      jest.spyOn(window, 'removeEventListener');
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { } });
      FinicityConnect.destroy();
    ...
    test('should close popup window', () => {
      ...
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { } }, { popup: true });
      FinicityConnect.destroy();
  ...
  describe('launch', () => {
    test('should handle popup scenario with default options', () => {
      ...
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { }, loaded }, { popup: true });
      expect(window.open).toHaveBeenCalledWith(url, 'targetWindow', `toolbar=${defaultPopupOptions.toolbar},location=${defaultPopupOptions.location},status=${defaultPopupOptions.status},menubar=${defaultPopupOptions.menubar},width=${CONNECT_POPUP_WIDTH},height=${CONNECT_POPUP_HEIGHT},top=${defaultPopupOptions.top},left=${defaultPopupOptions.left}`);
    ...
    test('should handle popup scenario with specified options', () => {
      ...
      const loaded = jest.fn();
      spyOn(FinicityConnect, 'initPostMessage').and.callFake(() => { });
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { }, loaded }, { popup: true, popupOptions });
      expect(loaded).toHaveBeenCalled();
      expect(window.open).toHaveBeenCalledWith(url, 'targetWindow', `toolbar=${popupOptions.toolbar},location=${popupOptions.location},status=${popupOptions.status},menubar=${popupOptions.menubar},width=${CONNECT_POPUP_WIDTH},height=${CONNECT_POPUP_HEIGHT},top=${popupOptions.top},left=${popupOptions.left}`);
      expect(FinicityConnect.initPostMessage).toHaveBeenCalled();
    });
    ...
    test('should handle iframe scenario with no overrides', () => {
      ...
      const loaded = jest.fn();
      spyOn(FinicityConnect, 'initPostMessage').and.callFake(() => { });
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { }, loaded });
    ...
    test('should handle iframe scenario with custom overlay and container', () => {
      ...
      const loaded = jest.fn();
      spyOn(FinicityConnect, 'initPostMessage').and.callFake(() => { });
      FinicityConnect.launch(url, { done: () => { }, error: () => { }, cancel: () => { }, loaded }, options);
    ...
    test('should log warning and append iframe to body if selector doesn\'t return an element', () => {
      ...
      expect(document.querySelector).toHaveBeenCalledWith('#container');
      expect(document.body.appendChild).toHaveBeenCalledWith(iframeStub);
      expect(console.warn).toHaveBeenCalledWith(`Couldn't find any elements matching "${options.selector}", appending "iframe" to "body" instead.`);
    });
  ...
  describe('initPostMessage', () => {
    test('should call postMessage every second with the expected parameters and attach postMessage event handler', () => {
      ...
      expect(FinicityConnect.postMessage).toHaveBeenCalledWith({ type: PING_EVENT, selector: '#container', sdkVersion: CONNECT_SDK_VERSION, platform: `${PLATFORM}-iframe` });


      FinicityConnect.initPostMessage({ popup: true });
      jest.advanceTimersByTime(1100);
      expect(FinicityConnect.postMessage).toHaveBeenCalledWith({ type: PING_EVENT, selector: undefined, sdkVersion: CONNECT_SDK_VERSION, platform: `${PLATFORM}-popup` });
    });

    test('should call attach postMessage event handler and send events as expected', () => {
      let eventHandler;
      spyOn(window, 'addEventListener').and.callFake((eventType, eh) => eventHandler = eh);
      const eventHandlers = { done: jest.fn(), error: jest.fn(), cancel: jest.fn() };
      ...
      eventHandler({ origin: url, data: { type: URL_EVENT, url: 'http://oauth.com' } });
      expect(FinicityConnect.openPopupWindow).toHaveBeenCalledWith('http://oauth.com');
      ...
      eventHandler({ origin: url, data: { type: CANCEL_EVENT, data: payload } });
  ...
  describe('openPopupWindow', () => {
    ...
    test('should call postMessage when the popup is closed', () => {
      ...
      jest.advanceTimersByTime(1100);
      expect(window.clearInterval).toHaveBeenCalled();
      expect(FinicityConnect.postMessage).toHaveBeenCalledWith({ type: WINDOW_EVENT, closed: true });
    });
  });

  describe('postMessage', () => {
    test('should call postMessage on (iframe)', () => {
      const iframeStub: any = { parentNode: { removeChild: jest.fn() }, setAttribute: jest.fn(), contentWindow: { postMessage: jest.fn() } };
      const metaStub: any = { parentNode: { removeChild: jest.fn() }, setAttribute: jest.fn() };
      spyOn(document.head, 'appendChild').and.callFake(() => { });
      spyOn(document.body, 'appendChild').and.callFake(() => { });
      spyOn(window.document, 'createElement').and.callFake((element) => element === 'iframe' ? iframeStub : metaStub);
